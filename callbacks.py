...
class MakeLRGreatAgain(Callback):
    """Increase learning rate when the loss function seems to be converged.
    ...
    Reducing the learning rate by a factor as in ReduceLROnPlateau seems to 
    be the way to stagnate the learning. However, this callback pushes the
    Learning Rate up high again, to make sure we don't get more improvements
    on our training. Making Learning Rate great again. 

    # Example

    ```python
    increase_lr = MakeLRGreatAgain(monitor='val_loss', factor_min=10, factor_max=1000,
                                  patience=10, min_lr=0.001)
    model.fit(X_train, Y_train, callbacks=[increase_lr])
    ```

    # Arguments
        monitor: quantity to be monitored.
        
        factor_min: minimum int to give the numpy.random.randint function.
            this throws a random factor by which the learning rate will
            be increased. new_lr = lr * factor
        factor_max: maximum int to give the numpy.random.randint function.
            this throws a random factor by which the learning rate will
            be increased. new_lr = lr * factor
        patience: number of epochs with no improvement
            after which learning rate will be reduced.
        ...
        cooldown: number of epochs to wait before resuming
            normal operation after lr has been reduced.
        min_lr: lower bound on the learning rate. When learning rate is less
            than min_lr, this callback pushes the Learning Rate up again.
    """

    def __init__(self, monitor='val_loss', factor_min=10, factor_max=1000, patience=10,
                 verbose=0, mode='auto', min_delta=1e-4, cooldown=0, min_lr=0, # min_delta might need to be increased (!)
                 **kwargs):
        super(MakeLRGreatAgain, self).__init__()

        self.monitor = monitor
        if factor_min <= 1.0:
            raise ValueError('MakeLRGreatAgain '
                             'does not support a factor_min <= 1.0 as you\'re not increasing the Learning Rate!')
        if factor_max <= 2.0:
            raise ValueError('MakeLRGreatAgain '
                             'does not support a factor_max <= 2.0 as you\'re not increasing the Learning Rate!')
        if 'epsilon' in kwargs:
            min_delta = kwargs.pop('epsilon')
            warnings.warn('`epsilon` argument is deprecated and '
                          'will be removed, use `min_delta` instead.')
    ...
    def _reset(self):
        ...
        """
        if self.mode not in ['auto', 'min', 'max']:
            warnings.warn('Learning Rate increasing mode %s is unknown, '
                          'fallback to auto mode.' % (self.mode),
                          RuntimeWarning)
            self.mode = 'auto'
        if (self.mode == 'min' or
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current is None:
            warnings.warn(
                'Increase LR conditioned on metric `%s` '
                'which is not available. Available metrics are: %s' %
                (self.monitor, ','.join(list(logs.keys()))), RuntimeWarning
            )
            ...
            if self.monitor_op(current, self.best):
                ...
                if self.wait >= self.patience:
                    old_lr = float(K.get_value(self.model.optimizer.lr))
                    if old_lr <= self.min_lr:
                        new_lr = old_lr * np.random.randint(low=self.factor_min, high=self.factor_max, size=None, dtype='l')
                        new_lr = max(new_lr, self.min_lr)
                        K.set_value(self.model.optimizer.lr, new_lr)
                        if self.verbose > 0:
                            print('\nEpoch %05d: MakeLRGreatAgain increasing '
                                  'learning rate to %s.' % (epoch + 1, new_lr))
                        self.cooldown_counter = self.cooldown
                        self.wait = 0
...
class BetterCSVLogger(Callback):
    """Callback that streams epoch results to a csv file.
    ...
    def __init__(self, filename, separator=',', append=False):
        ...
        if six.PY2:
            ...
            self.file_flags = ''
            self._open_args = {'newline': '\n'}
        super(BetterCSVLogger, self).__init__()
